...
//   }
  ...
  const adminUser = await User.findOne({
    ...
    const effectivePlanId = metadataPlanId || company.plan;

    if (!effectivePlanId) {
      throw new AppError("Plan not attached to company", 400, "PLAN_NOT_ATTACHED");
    }

    const plan = await Plan.findById(effectivePlanId);
    ...
    company.status =
      company.planStartAt && company.planExpiresAt ? "expired" : "payment_failed";
    ...
    if (adminUser && company.status === "payment_failed") {
